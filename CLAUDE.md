...
- `cargo xtask comment-lint` — filesystem walk that flags `kiro-XXXX` rivets-IDs, `PR #N`, and `issue #N` references inside `//` line comments under `crates/` and `xtask/`. Enforces the "no PR/issue refs in committed comments" rule below. Pattern matching is `//`-comment-scoped (block comments and string literals are skipped) and word-boundary aware (`mykiro-uphh`, `kiro-uphhx` don't match). Self-skips `xtask/src/comment_lint.rs` (its docstring documents the patterns it detects) and `xtask/src/plan_lint.rs` (its gate-query rationale is anchored to originating PRs). Other deliberate exceptions register in `comment_lint::ALLOWED_SITES` with a written-down reason. Exits 1 on findings.
...
- **No PR numbers, rivets-IDs, or issue numbers in committed comments.** This rule binds *all* `//` line comments under `crates/` and `xtask/` — including production code, tests, doc-comments (`///`, `//!`), and frontmatter. Forbidden shapes: `kiro-XXXX` (4-char rivets IDs like `kiro-uphh`), `PR #N` / `pr#N` (case-insensitive), and `issue #N`. Why: the referenced PR/issue is the *contemporaneous* changelog entry. Once the issue closes and rivets archives it, the in-code reference is a stale pointer the next reviewer has to grep `.rivets/issues.jsonl` (often gone) or GitHub for. PR descriptions, commit messages, and `git blame` are the durable audit trail; comments that duplicate them rot. The same rule applies to coined PR-review phrases (`"silent-install gap from PR #119 review"`) — describe the invariant, not its origin. **Enforced by `cargo xtask comment-lint`**. Deliberate exceptions (e.g. plan_lint gate rationale anchored to its originating PR) register in `xtask/src/comment_lint.rs`'s `ALLOWED_SITES` with a reason; the gate's own module is self-skipped because its docstring necessarily quotes the patterns it detects. **Rationale comments that explain WHY a defensive measure exists are still valuable** — just describe the failure shape (`"refusing hardlinked source — exfiltration risk"`) rather than the historical commit (`"closes kiro-uphh from PR #119 review"`).
