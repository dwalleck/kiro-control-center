...
7. **[debug_assert! in Constructors](#7-debug_assert-in-constructors)** - Catch bugs in development
...
## 7. debug_assert! in Constructors

### The Problem

Public structs with all public fields allow invalid states. Even with validation elsewhere, nothing prevents creating an invalid instance directly.

```rust
// ❌ RISKY - All public fields, no validation
pub struct ParsedFileData {
    pub relative_path: PathBuf,  // Could be absolute!
    pub line: u32,               // Could be 0 (invalid for 1-indexed)
    pub name: String,            // Could be empty!
}

// Anyone can create invalid instances:
let bad = ParsedFileData {
    relative_path: PathBuf::from("/absolute/path"),  // Bug!
    line: 0,  // Bug!
    name: String::new(),  // Bug!
};
```

### Solution: Validated Constructors with debug_assert!

Use `debug_assert!` in constructors to catch bugs during development with zero runtime cost in release builds.
...
impl ParsedFileData {
    ...
    pub fn new(
        ...
        // These only run in debug builds - zero cost in release
        debug_assert!(
        ...
        Self { relative_path, line, name }
    }
}
```

### When to Use debug_assert! vs assert! vs Result

| Approach | Runtime Cost | Use When |
|----------|--------------|----------|
| `debug_assert!` | Zero in release | Internal invariants, development checks |
| `assert!` | Always runs | Critical invariants that must never fail |
| `Result<T, E>` | Always runs | User input, external data, recoverable errors |

**Use `debug_assert!` for:**
- Invariants that "should never happen" if code is correct
- Checks that would be expensive to run in production
- Catching programmer errors during development

**Use `assert!` for:**
- Security-critical invariants
- Invariants where failure indicates memory corruption
- Cases where continuing would be dangerous

**Use `Result` for:**
- User-provided input
- Data from external sources (files, network, etc.)
- Recoverable error conditions

### Real Example from Parallel Parsing
...
1. **Catches bugs early** - Fails immediately during development/testing
2. **Zero release overhead** - Completely compiled out in release builds
3. **Documents invariants** - The assertions serve as executable documentation
4. **Guides API usage** - Callers see the constructor and use it correctly

### The Rule

**Add `debug_assert!` checks in constructors to validate invariants. This catches bugs during development with zero cost in production. For user input or external data, use `Result<T, E>` instead.**
...
### Fundamentals
- **[CLI user feedback](fundamentals.md#cli-user-feedback)** - Helpful user messages
- **[Validation patterns](fundamentals.md)** - General validation strategies

### Performance
- **[HashMap optimization](performance.md)** - Enum vs String performance
