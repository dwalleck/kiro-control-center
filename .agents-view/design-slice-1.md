...
| C7 | Project-only commands | `grep -E '(make_service\|MarketplaceService)' crates/kiro-control-center/src-tauri/src/commands/agents_authoring.rs`. Expected: zero matches. | static grep | 1m | **pending** (file does not exist yet — runs at implementation gate) | the grep itself, wired into CI as `cargo xtask plan-lint --gate no-marketplace-service-in-agents-authoring` (issue **kiro-6g6r**, filed below) |
...
3. **Falsifier non-vacuity.** For each claim, I name below a specific buggy implementation that the falsifier kills:
   ...
   - C7: if a future contributor adds `let svc = make_service()?` to a command body to share a helper, the grep finds it.
   - C8: if a wire-format type forgets `#[cfg_attr(feature = "specta", derive(specta::Type))]`, `bindings.ts` regen fails.
4. **Per-claim verification distinctness.** Every claim has a distinct test name OR a distinct oracle. If C2's falsifier fails, the error is "no-name file missing from probe output" — not interchangeable with any other claim's failure. C7's grep names the file and pattern explicitly.
5. **Cost distribution.** Cheapest 1m (C7 grep — runs at implementation gate). C2 at 5m. All others ≤ 20m. No claim has only an expensive (hours/days) falsifier.
