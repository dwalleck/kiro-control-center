...
| 14 | What `name` does the list-row carry when the JSON file's `name` field is absent or disagrees with the filename stem? | **REVISED 2026-05-22** — use the filename stem as the canonical identity. The JSON's `name` field is NOT used for the row's `name` value. Save and create paths enforce `JSON.name == filename` so the editor cannot introduce drift. | Original D14 (2026-05-21) said "JSON name first" to keep drift visible. claude[bot]'s PR #120 review surfaced that this choice broke downstream CRUD: `delete_user_agent`/`duplicate_user_agent`/`save_user_agent` all compute paths from `name`, so drifted rows could not be acted on (delete became silent no-op, duplicate returned `SourceNotFound`, save created a parallel file). New policy: row.name = filename stem always (canonical identity = path-stem); the `enforce_draft_json_name_matches` helper in `KiroProject` rejects any save/create whose payload disagrees. UI-side drift-visibility (a warning badge for files whose JSON-name differs from filename) is tracked at **kiro-78io**. |
